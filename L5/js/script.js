...
var boardElem; // Referens till div-element för spelplan
const allCarImgs = [
...
const yStep = 5; // Antal pixlar som bilen ska förflytta sig i y-led 
const timerStep = 20; // Tid i ms mellan varje steg i förflyttning
var timerRef = null; // Referens till timern för bilens förflyttning
var startBtn; // Referens till startknapp
var stopBtn; // Referens till stoppknapp
var carMenu; // Referens till menyn för att välja bil

/* ===== Tillägg i labben ===== */
var pigElem; //referar till grisen
var pigTimerRef = null; //Stoppar timern från att starta.
const pigDuration = 2000; //En ny gris varanan sekund.
var pigNr; //Antal grisar som har kommit fram.
var hitCounter; //Antal grisar som är träffade.
var pigNrElem; //Referens till antal räknade grisar.
var hitCounterElem; //Referens till antal grisar som är träffade.
var catchedPig; //Referens till träffade grisen.
// --------------------------------------------------
// Bilderna laddas in i förväg, så att alla bilder finns i webbläsarens cache
...
function startGame() {
	...
	document.activeElement.blur(); /* Knapparna sätts ur focus, så att webbsidan kommer i fokus igen för att man ska kunna känna av händelsen keydown i Firefox */
...
function newPig() {
	if (pigNr < 10) {
		let xLimit = boardElem.offsetWidth - pigElem.offsetWidth - 20; //Bestämmer grisens placering på bredden inom bordern.
		let yLimit = boardElem.offsetHeight - pigElem.offsetHeight - 20; //Bestämmer grisens placering på höjden inom bordern.
		let x = Math.floor(xLimit * Math.random()) + 10; //placeringen flyttar grisen på bredden med 10px
		let y = Math.floor(yLimit * Math.random()) + 10; //placeringen flyttar grisen på höjden med 10px
...
// Talar om att grisen har blivit träffad.
function checkHit() {
	if (catchedPig == true) {
		return;
	}
	let cSize = carElem.offsetWidth; //sparar bilens offset på bredden.
	let pSize = pigElem.offsetWidth; //sparar grisens offset på bredden.
	let cL = parseInt(carElem.style.left); //känner av om det blir en träff
